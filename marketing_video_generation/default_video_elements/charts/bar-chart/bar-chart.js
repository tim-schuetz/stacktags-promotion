...
(function (global) {
  ...
  class StacktagsGraphChart {
    constructor(host, opts = {}) {
      ...
      this.W = 900; this.H = 540; this.pad = { l: 40, r: 40, t: 30, b: 70 };
    ...
    _build() {
      ...
      if (this.data.some(d => d.label)) {
        ...
        this.data.forEach((d, i) => {
          if (!d.label) return;
          const tx = el('text', { x: this._x(i), y: this.H - b + 44, 'text-anchor': 'middle', 'font-size': 26 });
    ...
    _buildLine(svg) {
      ...
      this.area = el('path', { class: 'stk-chart-area', d: areaD });
    ...
    showAll() {
      ...
      else { this.clipRect.setAttribute('width', this.W); const p = this._pts[this._pts.length - 1]; [this.dot, this.halo].forEach(c => { c.setAttribute('cx', p[0]); c.setAttribute('cy', p[1]); c.setAttribute('opacity', 1); }); }
      const last = this.data.length ? this.data[this.data.length - 1].value : 0;
      const total = this.data.reduce((s, d) => s + d.value, 0);
      const headlineTarget = this.opts.headline != null ? this.opts.headline : (this.opts.type === 'bar' ? total : last);
