...
const END = 83.3;   // narration length (script_voice.mp3 ≈ 83.31s)

(async () => {
  const browser = await chromium.launch({
    executablePath: exe,
    headless: true,
    // No WebGL in this video (pure CSS animation) → no GPU flags needed; the
    // aggressive ones from the globe-based reference just add headless crash risk.
    args: [
  ...
  await page.waitForTimeout(3500);   // tail — hold on the assembled outro
