  ...
  class StacktagsDepthTransitionsOptimized {
    constructor(host, opts = {}) {
      ...
      this.layers = this.steps.map(s => {
        ...
        if (s.html !== undefined) {
          // arbitrary custom-content beat (cards, grids, equations …) — the
          // camera/grid math treats it exactly like any other beat.
          beat.classList.add('stk-depth2-custom');
          beat.innerHTML = s.html;
        } else if (s.image !== undefined || s.placeholder !== undefined) {
          if (s.image) {
    ...
    _poses(mode, e) {
      switch (mode) {
        ...
        case 'zoom-out':   // camera pulls back: out recedes small; in revealed from huge
          return {
            from: { s: lerp(1, 0.34, e), ty: lerp(0, 40, e),  blur: 14 * e,     op: clamp01(1.1 - e / 0.7), z: 1 },
            to:   { s: lerp(2.6, 1, e),  blur: 18 * (1 - e),  op: clamp01((e - 0.05) / 0.45), z: 3 },
            grid: 0.78,
          };
        ...
        default:           // camera pushes in: out balloons + blurs (lingers); in emerges from below-small
          return {
            from: { s: lerp(1, 3.0, e), blur: 24 * e,         op: clamp01(1.1 - e / 0.62), z: 3 },
            to:   { s: lerp(0.35, 1, e), ty: lerp(240, 0, e), blur: 8 * (1 - e), op: clamp01((e - 0.05) / 0.45), z: 2 },
            grid: 1.45,
          };
      }
    }
    ...
    /* Jump straight to beat `i` at its rest anchor with a neutral grid — used
       for headless seeking / QA screenshots (no camera animation). */
    seekTo(i) { this.reset(); this._showFirst(i); }
