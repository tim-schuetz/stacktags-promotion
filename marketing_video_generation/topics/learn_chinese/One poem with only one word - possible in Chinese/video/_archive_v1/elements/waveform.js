/* ============================================================
   ...
   A teal audio waveform that animates by itself. Modes:
     'pop'  — energetic, many short bars (a lively pop track)
     'open' — a few long, held, smooth open-vowel lobes
     'flat' — a lifeless, identical, muted-gray blur (every bar the
              same height) — for "one flat, identical wall of sound"
   USAGE
...
(function (global) {
  function mountWaveform(host) {
    ...
    function draw(now) {
      ...
      } else if (mode === 'open') { // long held lobes
        const N = 7;
        const seg = W / N;
        for (let i = 0; i < N; i++) {
      ...
      } else { // 'flat' — every bar identical, muted, barely alive
        const N = Math.max(28, Math.floor(W / 26));
        const gap = W / N;
        const bw = gap * 0.46;
        // ONE height for all bars — a lifeless, identical blur
        const h = (H * 0.13) * (0.92 + 0.08 * Math.sin(t * 2.4)) * env;
        for (let i = 0; i < N; i++) {
          const x = i * gap + gap / 2;
          const g = ctx.createLinearGradient(0, midY - h, 0, midY + h);
          g.addColorStop(0, '#c2cad0'); g.addColorStop(1, '#a6b0b8');
          ctx.fillStyle = g;
          roundRect(ctx, x - bw / 2, midY - h, bw, h * 2, bw / 2);
        }
      }
      // soft center line
      ctx.strokeStyle = mode === 'flat' ? 'rgba(140,148,157,0.22)' : 'rgba(62,174,147,0.18)';
      ctx.lineWidth = 2;
