...
const audioIn = fs.existsSync(sfxAudio) ? sfxAudio : mp3;   // narration + SFX if mixed, else bare voice
const out  = path.resolve(BUILD, '../5-chinese-words-you-already-use.mp4');
...
// Hold the final "Follow" end-card ~1.6s past the last spoken word:
// pad the narration with trailing silence and cap the output length.
const OUT_DUR = 127.3;   // audio ≈ 125.81s + ~1.5s outro hold
const args = [
  '-y',
