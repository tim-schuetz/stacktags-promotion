...
(function () {
  ...
  const SUBS = [
    ...
    [2.68, "but more and more of them can't <b>write</b> it by hand anymore."],
    [5.36, "There's even a name for it:"],
    [6.62, '<b>character amnesia</b>.'],
    [7.92, 'And the cause is sitting in your <b>pocket</b>.'],
    [9.88, 'Picture an educated Chinese speaker, pen in hand,'],
    [12.62, 'and they suddenly <b>blank</b>'],
    [13.52, 'on how to write a character they use every single day.'],
    [16.28, 'The Chinese term for it is'],
    [17.50, '<b>tí bǐ wàng zì</b>'],
    [19.12, '— literally, "pick up the pen, forget the character."'],
    [21.78, "And it's <b>spreading</b>. So what's going on?"],
    [24.06, 'It comes down to how you <b>type</b> Chinese...'],
    [26.18, "You don't draw the characters."],
    [28.02, 'You type the <b>sound</b>. The Pinyin.'],
    [29.66, 'And your phone shows you a list of matching characters.'],
    [32.32, 'You just <b>pick the right one</b>.'],
    [33.70, 'So all day long, you only have to <b>recognize</b> the correct character.'],
    [36.92, 'Never build it from memory, stroke by stroke.'],
    [39.50, 'And recognition is <b>easy</b>.'],
    [41.30, 'Active recall — producing the whole thing from scratch —'],
    [44.22, 'is <b>hard</b>.'],
    [45.92, 'Classic <b>use-it-or-lose-it</b>.'],
    [47.48, 'People read fluently, type fluently,'],
    [49.54, 'then freeze the moment they have to handwrite.'],
    [51.56, 'Even simple, everyday characters <b>slip away</b>.'],
    [54.20, 'For thousands of years, to truly know a character'],
    [56.16, 'was to be able to <b>write</b> it.'],
    [57.22, 'Now, a billion people know their characters perfectly,'],
    [60.16, "and just can't quite <b>draw</b> them anymore."],
    [62.28, 'Maybe the first writing system in history'],
    [63.72, 'quietly handed over to a <b>keyboard</b>...'],
    [65.86, 'Want to actually start learning <b>Chinese</b>?'],
    [68.04, 'Discover thousands of <b>free</b> exercises'],
    [69.80, 'and more learning content on <b>stacktags.io</b>.'],
  ];
  ...
  const CUES = [
    ...
    [7.92, () => $('#hk-phone').classList.add('in')],
    [9.15, () => { const p = $('#hk-phone'); p.classList.remove('in'); p.classList.add('out'); }],

    // PHENOMENON — picture an educated Chinese speaker (a real student at a board)
    [9.88, (i) => enter($('#sc-student'), 'rise', 950, i)],

    // PHENOMENON 2 — the taught idiom 提笔忘字
    [16.28, (i) => enter($('#sc-term'), 'drop', 1000, i, () => { if (i && termPop) termPop.showAll(); })],
    [17.50, (i) => { if (!i && termPop) termPop.pop(0); }],
    [17.86, (i) => { if (!i && termPop) termPop.pop(1); }],
    [18.10, (i) => { if (!i && termPop) termPop.pop(2); }],
    [18.40, (i) => { if (!i && termPop) termPop.pop(3); }],
    [19.12, () => $('#term-py').classList.add('in')],
    [20.30, () => $('#term-gloss').classList.add('in')],

    // WHY — the phone IME does the recognising
    [24.06, (i) => enter($('#sc-phone'), 'pan-right', 1050, i)],
    [28.60, (i) => imeType(i)],
    [30.30, (i) => imeCands(i)],
    [32.32, (i) => imeTap(i)],

    // RECALL — recognition stays (ghost + phone), recall fades (ink crumbles in waves)
    [33.70, (i) => { enter($('#sc-recall'), 'zoom-in', 1050, i); hzRecall.full(); }],
    [36.92, (i) => hzRecall.crumbleList([15, 13, 11], { instant: i, stagger: 120 })],
    [39.50, (i) => { $('#rc-phone').classList.add('lit'); hzRecall.restore(); }],
    [41.30, (i) => { $('#rc-phone').classList.remove('lit'); hzRecall.crumbleList([15, 14, 13, 12, 11, 10], { instant: i, stagger: 95 }); }],
    [45.92, (i) => hzRecall.crumbleList([9, 8, 7, 6], { instant: i, stagger: 110 })],
    [49.54, (i) => hzRecall.crumbleList([5, 4, 3], { instant: i, stagger: 120 })],
    [52.60, (i) => hzRecall.crumbleList([2, 1, 0], { instant: i, stagger: 200 })],
    ...
    [54.20, (i) => enter($('#sc-history'), 'zoom-out', 1150, i)],
    [55.22, (i) => hzHistory.assemble({ instant: i, stagger: 95 })],
    [60.16, (i) => hzHistory.crumble({ instant: i, stagger: 85 })],
    [62.28, () => $('#hs-phone').classList.add('in')],
    [64.10, (i) => histType(i)],

    // OUTRO
    [65.86, (i) => { enter($('#sc-outro'), 'lift', 1100, i); outroAssemble(); }],
  ];
  ...
  const SFX = [
    [9.88, 'swoosh', 0.50],
    [16.28, 'swoosh', 0.50],
    [17.50, 'pop', 0.50], [17.86, 'pop', 0.50], [18.10, 'pop', 0.50], [18.40, 'pop', 0.50],
    [24.06, 'swoosh', 0.50],
    [32.32, 'pop', 0.45],
    [33.70, 'swoosh', 0.50],
    [39.50, 'pop', 0.45],
