...
(function () {
  ...
  window.mountStacktagsGlobe = function (el, opts = {}) {
    ...
    let gloRunning = true, gloPaused = false;   // render gate (keeps capture real-time)
    ...
    let markerRing = null, markerDot = null, markerGroup = null;
    ...
    async function init() {
      ...
      el.__globeCtrl = {
        ...
        // render gate — software WebGL is expensive, so the host can stop the
        // globe from rendering when it's off-screen to keep capture real-time.
        pause() { gloPaused = true; },
        resume() { gloPaused = false; },
        // snap the camera to a view instantly (no animation) — used to start a
        // transition already framed on a city.
        setView(lat, lon, cam) {
          state.lat = lat; state.lon = lon; if (cam != null) state.camDist = cam;
          state.tLat = lat; state.tLon = lon; if (cam != null) state.tCam = cam;
        },
        // move the grey marker to a new city and re-pop it (for a 2nd dive).
        moveMarker(lat, lon) {
          if (markerGroup && globe) globe.remove(markerGroup);
          markerGroup = buildMarker(lat, lon);   // reassigns markerDot/markerRing at new pos
          if (globe) globe.add(markerGroup);
          const s0 = performance.now();
          (function popM(t) {
            const p = Math.min(1, (t - s0) / 440);
            const e = p < 1 ? 1 + (2.7 * Math.pow(p - 1, 3) + 1.7 * Math.pow(p - 1, 2)) : 1;
            if (markerDot) markerDot.scale.setScalar(Math.max(0.0001, e));
            if (p < 1) requestAnimationFrame(popM);
          })(performance.now());
          pingActive = true; pingT0 = performance.now() + 120;
        },
        dispose() {
          gloRunning = false;
    ...
    function animate(now) {
      if (!gloRunning) return;            // disposed — stop the loop entirely
      requestAnimationFrame(animate);
      if (gloPaused) return;              // off-screen — skip the expensive render
      if (zoom.active) {
