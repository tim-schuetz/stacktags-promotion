# Cut out Liu Bannong (rembg) then bake a dashed turquoise outline -> liu_dashed.png.
import os, math
import numpy as np
...
# --- 1) rembg cut, tight crop ---
inp = Image.open(os.path.join(IMG, 'liu_raw.png')).convert('RGBA')
cut = remove(inp, alpha_matting=True, alpha_matting_foreground_threshold=240, alpha_matting_background_threshold=15)
bb = cut.getbbox()
if bb:
    cut = cut.crop(bb)
cut.save(os.path.join(IMG, 'liu_cut.png'))
print('cut', cut.size)
...
out.save(os.path.join(IMG, 'liu_dashed.png'))

prev = Image.new('RGBA', out.size, (255, 255, 255, 255))
prev.paste(out, (0, 0), out)
os.makedirs(os.path.join(HERE, 'capture'), exist_ok=True)
prev.convert('RGB').save(os.path.join(HERE, 'capture', 'liu_dash_preview.png'))
print('saved liu_dashed', out.size)
