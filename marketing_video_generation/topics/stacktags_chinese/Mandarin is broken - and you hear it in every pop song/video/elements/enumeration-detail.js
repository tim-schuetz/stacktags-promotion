...
(function (global) {
  ...
  class StacktagsEnumerationDetail {
    constructor(host, opts = {}) {
      ...
      this.rows = this.items.map((it) => {
        ...
        row.innerHTML =
          ...
          `</div>` +
          `<div class="se2-tile${it.glyph ? ' glyphtile' : ''}">` +
            `<div class="se2-tile-side"></div>` +
            `<div class="se2-tile-top"><span${it.glyph ? ' class="glyph"' : ''}>${it.glyph || 'Bild'}</span></div>` +
          `</div>`;
    ...
    /* Light a single card turquoise IN PLACE (no docking / no running title) —
       used to single out one item right before the camera zooms into it. */
    spotlight(index) { this.tiles.forEach((t, k) => t.classList.toggle('active', k === index));
      this.rows.forEach((r, k) => r.classList.toggle('dim', index >= 0 && k !== index)); }
