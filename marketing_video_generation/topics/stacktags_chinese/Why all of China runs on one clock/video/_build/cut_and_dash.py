# For each raw object photo: rembg cut-out (crop to object) → bake a DASHED
# silhouette outline → one transparent PNG. obj_<x>_raw.png -> obj_<x>_dash.png
import math, os
import numpy as np
...
OBJECTS = ['obj_bus', 'obj_shop', 'obj_dinner']

def cutout(src):
    inp = Image.open(src).convert('RGBA')
    out = remove(inp, alpha_matting=True, alpha_matting_foreground_threshold=240,
...
def dash(img):
    ...
    return canvas.resize((W + 2 * P, H + 2 * P), Image.LANCZOS)
...
for key in OBJECTS:
    ...
    cut = cutout(src)
    cut.save(os.path.join(PH, key + '_cut.png'))
    out = dash(cut)
    out.save(os.path.join(PH, key + '_dash.png'))
    print('wrote', key + '_dash.png', out.size)
