...
// from window.SFX) and overlay each cue onto script_audio.mp3 with ffmpeg:
...
const SND = {
  ...
  ticking: path.resolve(BUILD, '../assets/sound/tickingtimeline.mp3'),
};
...
// Everything (narration + both SFX) is MONO — keep it mono. Forcing a stereo
// layout would split the mono energy through the upmix pan-law and quietly drop
// the narration ~3 dB. amix(normalize=0) sums without dividing, so the voice
// stays at its original level and the sounds add on top. A transparent safety
// limiter guards against clipping if SFX volumes are raised later.
