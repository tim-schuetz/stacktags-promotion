# Cut out the generated images (rembg) and bake a dashed silhouette outline on
# the single objects. Final transparent PNGs land in ../assets/<key>.png
import math, os
import numpy as np
...
# key -> dashed? (single objects get the dashed outline; characters/buildings don't)
JOBS = {
    'cash_pile':        True,
    'vault':            True,
    'dollar_bill':      True,
...
for key, dashed in JOBS.items():
    img = cutout(key)
    if dashed:
        img = bake_dashed(img)
    img.save(os.path.join(ASSETS, key + '.png'))
    print('wrote', key + '.png', img.size, 'dashed' if dashed else 'cut')
print('done')
