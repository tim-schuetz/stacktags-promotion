// Forced alignment via OpenAI Whisper (verbose_json, word+segment timestamps).
// Output: whisper.json (raw). Whisper word-times are reliable across the clip.
const fs = require('fs');
...
const AUDIO = path.resolve(__dirname, '../../script_voice.mp3');

(async () => {
  const buf = fs.readFileSync(AUDIO);
  const fd = new FormData();
  fd.append('file', new Blob([buf], { type: 'audio/mpeg' }), 'script_voice.mp3');
