...
const out = path.resolve(BUILD, '../tether-genius-business-model.mp4');
...
console.log('preroll trim =', preroll.toFixed(3), 's  | audio =', path.basename(audio));
...
const args = [
  ...
  '-c:a', 'aac', '-b:a', '192k',
  // pad with silence so the outro brand card finishes assembling + holds a beat
  '-af', 'apad', '-t', '123.6',
  '-movflags', '+faststart',
  out,
];
