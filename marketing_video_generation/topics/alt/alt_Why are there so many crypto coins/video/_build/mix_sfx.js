...
// Reads the page's DECLARED sound bed (capture/sfx.json from window.SFX) and
// overlays each cue onto script_voice.mp3 with ffmpeg: each sound delayed to
// its narration time t, scaled to its vol, summed (normalize=0 so the voice
// keeps full level) and soft-limited. Output → capture/narration_sfx.m4a.
...
// Everything (narration + SFX) is MONO — keep it mono so the voice level is
// preserved (a stereo upmix would drop it ~3 dB). amix(normalize=0) sums.
