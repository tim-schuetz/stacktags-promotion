// ============================================================
// Stacktags 3D globe — vanilla port of the app's MapView.vue (Three.js).
// Renders the real world-atlas geometry as a white globe with country
// outlines, shown as a full 3D globe. Germany lights up teal a moment after
// the globe settles — the dynamic "Click on Germany." map task in scene 9.
//
// Depends on globals: THREE, earcut, topojson (loaded via CDN <script>).
// Mounts lazily (only when scene 9 is shown) via window.mountStacktagsGlobe.
// ============================================================
(function () {
  const GLOBE_RADIUS = 1;
  const TEXTURE_SIZE = 2048;
  // Centre the view on Europe so Germany reads clearly in the upper-middle
  // of the sphere (not a tiny dot above Africa) while the whole globe still
  // shows. Matches the "Click on Germany." reference.
  const FOCUS = { lat: 32, lon: 11, cam: 2.45 };
  const HIGHLIGHT_NAME = 'germany';
  ...
  function drawGeo(ctx, geojson, W, H, opts) {
    forEachRing(geojson, (ring, props, isOuter) => {
      const subRings = [[]];
  ...
  window.mountStacktagsGlobe = function (el) {
    if (!el || el.querySelector('canvas')) return;
    if (!window.THREE || !window.earcut || !window.topojson) return; // CDN not ready

    let renderer, scene, camera, globe, animId;
    const state = { lat: 20, lon: FOCUS.lon, camDist: 3.2, tLat: FOCUS.lat, tLon: FOCUS.lon, tCam: FOCUS.cam };
    function size() {
      const r = el.getBoundingClientRect();
      // Always render into a SQUARE viewport (side = smaller dimension) so the
      // sphere is a true circle — never stretched to a wide container.
      const side = Math.max(Math.min(r.width || 300, r.height || 300), 160);
    ...
    function paintTexture(world, ctx, canvas) {
      ...
      drawGeo(ctx, world, W, H, { fill: '#ffffff', stroke: 'rgba(0,0,0,0.30)', lineWidth: 1.1 });
    }

    function buildHighlight(world, name) {
      const target = String(name || HIGHLIGHT_NAME).toLowerCase();
      ...
      if (fillPositions.length) {
        ...
        grp.add(new THREE.Mesh(fg, new THREE.MeshBasicMaterial({ color: 0x3eae93, transparent: true, opacity: 0.5, side: THREE.DoubleSide, depthWrite: false })));
      }
      return grp;
    }
    ...
    async function init() {
      ...
      try {
        ...
        // Fade a freshly-built country highlight in over ~500ms, fading any
        // previous highlight out first.
        let currentHl = null;
        function revealCountry(name, delay) {
          ...
          setTimeout(() => {
            if (currentHl) {
              const old = currentHl;
              const om = [];
              old.traverse(o => { if (o.material) om.push({ m: o.material, from: o.material.opacity }); });
              const t1 = performance.now();
              (function fade(t){
                const p = Math.min(1, (t - t1) / 400);
                om.forEach(({ m, from }) => { m.opacity = from * (1 - p); });
                if (p < 1) requestAnimationFrame(fade); else globe.remove(old);
              })(performance.now());
            }
            hl.visible = true;
            currentHl = hl;
        ...
        // Dynamic reveal: a moment after the globe settles, Germany lights up.
        revealCountry('germany', 1500);

        // Controller so scene 9 can advance to a 2nd map task — rotate the
        // globe to another country and highlight it.
        el.__globeCtrl = {
          focusCountry(name, lat, lon, cam) {
            state.tLat = lat; state.tLon = lon; if (cam) state.tCam = cam;
            revealCountry(name, 350);
          }
        };
